...
function CustomizedMenus(props) {
  ...
  const onFileUpload = () => {
    ...
    axios({
      method: "post",
      url: `https://14.102.108.122:3333/upload?ping=${localStorage.getItem(
        "ping"
      )}`,
      headers: {
      ...
      .then((response) => {
        if (response.data.newtoken) {
          ...
          // axios({
          //   method: "post",
          //   url: `https://14.102.108.122:3333/updatedata?ping=${localStorage.getItem(
          //     "ping"
          //   )}`,
          //   headers: {
          //     "Content-type": "application/json",
  ...
  const onIpfsUpload = async () => {
    ...
    axios({
      method: "post",
      url: `https://14.102.108.122:3333/ipfs/upload`,
      headers: {
        "Content-type": "multipart/form-data",
        Authtoken: at,
        verificationToken: enc,
